...
def usage_and_exit():
    ...
    print "                the executable name is 'yaniv'."
    sys.exit(1)
...
for dir in input_dirs:
    ...
    if INPUT:
        ...
        os.environ["YANIV_TESTS_DIR"] = dir
        os.system(TEST_FILES_CMD)
        del os.environ["YANIV_TESTS_DIR"]
    if VALGRIND:
        print "*** Tests from %s (valgrind): ***" % dir
        os.environ["YANIV_TESTS_DIR"] = dir
        os.environ["YANIV_RUN_VALGRIND"] = "1"
        os.system(TEST_FILES_CMD)
        del os.environ["YANIV_RUN_VALGRIND"]
        del os.environ["YANIV_TESTS_DIR"]
